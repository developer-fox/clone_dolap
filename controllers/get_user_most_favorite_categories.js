...
module.exports.forLooking = async(user_id)=>{
  try {    
	  const user = await user_model.findById(req.decoded.id).select("user_looked_notices").populate("user_looked_notices","details");
...
module.exports.forSaling = async(user_id)=>{
  try {    
	  const user = await user_model.findById(req.decoded.id).select("notices").populate("notices","details");
    let bottomCategoriesWithCount = {};
